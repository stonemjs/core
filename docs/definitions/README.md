[**Core Documentation v0.0.33**](../README.md) • **Docs**

***

[Core Documentation v0.0.33](../modules.md) / definitions

# definitions

## Index

### Enumerations

- [LogLevel](enumerations/LogLevel.md)

### Interfaces

- [AdapterContext](interfaces/AdapterContext.md)
- [AdapterHooks](interfaces/AdapterHooks.md)
- [ConfigContext](interfaces/ConfigContext.md)
- [ErrorOptions](interfaces/ErrorOptions.md)
- [ErrorResponse](interfaces/ErrorResponse.md)
- [IAdapter](interfaces/IAdapter.md)
- [IAdapterEventBuilder](interfaces/IAdapterEventBuilder.md)
- [IConfiguration](interfaces/IConfiguration.md)
- [IErrorHandler](interfaces/IErrorHandler.md)
- [IListener](interfaces/IListener.md)
- [ILogger](interfaces/ILogger.md)
- [IProvider](interfaces/IProvider.md)
- [IRawResponseWrapper](interfaces/IRawResponseWrapper.md)
- [IRouter](interfaces/IRouter.md)
- [ISubscriber](interfaces/ISubscriber.md)
- [KernelContext](interfaces/KernelContext.md)
- [LifecycleEventHandler](interfaces/LifecycleEventHandler.md)
- [RawResponseOptions](interfaces/RawResponseOptions.md)

### Type Aliases

- [AdapterHandlerResolver](type-aliases/AdapterHandlerResolver.md)
- [AdapterListenerHook](type-aliases/AdapterListenerHook.md)
- [AdapterResolver](type-aliases/AdapterResolver.md)
- [ClassMethodType](type-aliases/ClassMethodType.md)
- [ClassType](type-aliases/ClassType.md)
- [ConfigModuleName](type-aliases/ConfigModuleName.md)
- [ConfigRawModules](type-aliases/ConfigRawModules.md)
- [DeepPartial](type-aliases/DeepPartial.md)
- [ErrorHandlerLevels](type-aliases/ErrorHandlerLevels.md)
- [ErrorHandlerRenderResponseResolver](type-aliases/ErrorHandlerRenderResponseResolver.md)
- [ErrorHandlerResolver](type-aliases/ErrorHandlerResolver.md)
- [EventHandler](type-aliases/EventHandler.md)
- [EventHandlerFunction](type-aliases/EventHandlerFunction.md)
- [IBlueprint](type-aliases/IBlueprint.md)
- [KernelResolver](type-aliases/KernelResolver.md)
- [LoggerResolver](type-aliases/LoggerResolver.md)
